...
// center frequency of band m (same formula as the input layer)
...
// bilinear sample of a grid array (stride 3, component c)
    ...
    const processHops = (maxHops: number): number => {
      ...
      for (let hopN = 0; hopN < maxHops; hopN++) {
        ...
        for (let i = 0; i < HOP_SAMPLES; i++, idx++) {
          ...
          for (let m = 0; m < M; m++) {
            ...
            env2[iL] += aE * (yL * yL - env2[iL]) // per-band leaky integration (cochlear envelope)
      ...
      for (let i = 0; i < N; i++) {
        ...
        if (fired) {
          ...
          // neighborhood cooperation (topographic; radius/strength converge via annealing)
          ...
          for (let ny = y0; ny <= y1g; ny += nbStep) {
            for (let nx = x0; nx <= x1g; nx += nbStep) {
              ...
              if (h > 0.0008) updateCellVisual(j) // skip color updates for distant cells whose change is negligible
            }
          }
        }
      }
